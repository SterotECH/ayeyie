<x-ui.admin-page-layout
    :title="'Pickup Details #' . $pickup->pickup_id"
    description="View comprehensive pickup information and order details"
    :breadcrumbs="[
        ['label' => 'Dashboard', 'url' => route('dashboard')],
        ['label' => 'Pickups', 'url' => route('customers.pickups.index')],
        ['label' => 'Pickup #' . $pickup->pickup_id]
    ]"
    :stats="[
...
>
    <x-slot:actions>
        <div class="flex items-center gap-3">
            ...
            @endif
            <flux:button href="{{ route('customers.pickups.index') }}" variant="ghost" icon="arrow-left">
                Back to Pickups
            </flux:button>
        </div>
    </x-slot:actions>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-card rounded-lg border border-border p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-semibold text-primary">Pickup Status</h3>
                    @if($pickup->pickup_status === 'pending')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            <dt class="text-sm font-medium text-secondary">Pickup ID</dt>
                            <dd class="mt-1 text-lg font-semibold text-primary">#{{ $pickup->pickup_id }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-secondary">Receipt Code</dt>
                            <dd class="mt-1 text-sm text-primary font-mono">{{ $pickup->receipt->receipt_code }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-secondary">Order Number</dt>
                            <dd class="mt-1 text-sm text-primary">#{{ $pickup->receipt->transaction->transaction_id }}</dd>
                        </div>
                    </div>
                    <div class="space-y-4">
                        <div>
                            <dt class="text-sm font-medium text-secondary">Pickup Date</dt>
                            <dd class="mt-1 text-sm text-primary">
                                @if($pickup->pickup_date)
                                    {{ $pickup->pickup_date->format('M j, Y g:i A') }}
                                @else
                                    <span class="text-muted">Not yet picked up</span>
                                @endif
                            </dd>
                        </div>
                        <div>
                            ...
                            <dd class="mt-1">
                                @if($pickup->user)
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm text-primary">{{ $pickup->user->name }}</span>
                                    </div>
                                @else
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-primary">{{ $pickup->created_at->format('M j, Y g:i A') }}</dd>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tfoot class="border-t border-border">
                            <tr>
                                ...
                                <td class="py-4 px-4 text-right text-lg font-bold text-primary">₵{{ number_format($pickup->receipt->transaction->total_amount, 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-card rounded-lg border border-border p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm text-primary">{{ $pickup->receipt->transaction->transaction_date->format('M j, Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm text-primary">{{ ucfirst($pickup->receipt->transaction->payment_method) }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm font-bold text-success">₵{{ number_format($pickup->receipt->transaction->total_amount, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            @if($pickup->pickup_status === 'pending')
                <div class="bg-warning/5 rounded-lg border border-warning/20 p-6">
                    ...
                    <div class="space-y-3 text-sm text-secondary">
                        <p>• Please bring a valid ID for pickup verification</p>
                        <p>• Present the receipt code: <code class="bg-muted px-2 py-1 rounded font-mono">{{ $pickup->receipt->receipt_code }}</code></p>
                        <p>• Store hours: Monday - Friday, 8:00 AM - 6:00 PM</p>
            ...
            <div class="bg-card rounded-lg border border-border p-6">
                ...
                <div class="space-y-3">
                    <flux:button href="{{ route('customers.orders.show', $pickup->receipt->transaction) }}" variant="outline" size="sm" class="w-full justify-start" wire:navigate>
                        <flux:icon name="shopping-bag" class="size-4 mr-2" />
                        View Original Order
                    </flux:button>
